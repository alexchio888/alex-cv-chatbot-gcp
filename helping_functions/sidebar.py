...
def _render_contact():
    ...
    for contact in contact_links:
        st.sidebar.markdown(
            ...
            f'style="text-decoration:none; display:inline-flex; align-items:center; white-space:nowrap; margin: 2px 0; color: #3399FF;">'
            f'<img src="{contact["icon"]}" width="20" style="margin-right:8px; text-decoration:none;" /> '
            f'<span style="text-decoration:none;">{contact["label"]}</span></a>',
            unsafe_allow_html=True,
        )
    # Download CV link
    cv_html = """
